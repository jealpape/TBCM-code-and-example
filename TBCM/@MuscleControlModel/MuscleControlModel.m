...
% Class for implementing the muscular control of the prephonatory laryngeal
% posture and prephonatory strain-stress state in the vocal folds following
% the ideas introduced in [1] and in [2], Chapter 3. The model also
% implements the rules for computing the biomechanical variables/parameters
% (masses and spring constants) for the body cover model and the triangular
% the body cover model of the vocal folds [3]. 
% The MuscleControlModel allows the dynamic simulation of the laryngeal
% posture due to the coordinated activation of the five intrinsic muscles:
% cricothyroid (CT), thyroarytenoid (TA), lateral cricoarytenoid (LCA),
% posterior cricoarytenoid (LCA), and interarytenoid (IA); togheter with
% the vocal fold mucosa and ligament. Also, the model includes static
% methods for computing the geometric and mechanical parameters involved in
% body cover formulation as a function of CT, TA, and LC muscles.
%
% References:
...
%
% [3] I. R. Titze and B. H. Story, “Rules for controlling low-dimensional 
...
%     DEPTH_MUS_FEMALE = 0.3e-2; % [m] Depth of TA muscle (0.4 [cm] in males and 0.3 [cm] in females)
  ...
  properties (SetAccess = protected)
    ...
    Xi_PGO = 3.2e-3; % Poterior wall (half-width) x-position
    Psi_PGO = -2.5e-3; % Posterior wall y-position
  ...
  methods
    ...
    %           (+/-Xi_02, Psi_02) and (+/-Xi_PGO, Psi_PGO)
      ...
      xi_a = MCObj.xData_CAJ(1);
      xi_pnt = max(0,MCObj.Xi_PGO+xi_a); % MCObj.Xi_PGO+xi_a;
...
%     function theta_a = get.theta_a(MCObj)
%     % Function for retrieving the arithenoid angle.
%       theta_a = MCObj.xData_CAJ(7);
%     end
