...
%   Fdb = -TBCMobj.db*(1+TBCMobj.NonLinDamping*4.0e3*abs(xb-xb0+max(xu,xl)))*(vb);     % Damping force of body mass [N]
  Fdb = -TBCMobj.db*(1+TBCMobj.NonLinDamping*0.0e3*abs(max(deltaxu,deltaxl)))*(vb);     % Damping force of body mass [N]
%   Fdb = -TBCMobj.db*(1+TBCMobj.NonLinDamping*5.0e3*abs(deltaxb+max(Delta_half_u,Delta_half_l)))*(vb);     % Damping force of body mass [N]
end