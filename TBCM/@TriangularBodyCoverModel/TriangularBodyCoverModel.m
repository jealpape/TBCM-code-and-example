...
%       zetal = TBCMobj.zetal0 * (1.0+3.0*TBCMobj.alpha_l); 
    ...
    % Function for computing au the glottal area for the upper portion
      Delta_half_u = max([0, TBCMobj.xi_02])/2;
      xiu_f = TBCMobj.xData(1)-Delta_half_u-TBCMobj.xu_col;
      au = 2*max([0, (1-TBCMobj.alpha_u)*TBCMobj.Lg*(xiu_f+(1+TBCMobj.alpha_u)*Delta_half_u)]);
    end
    
    function al = get.al(TBCMobj)
    % Function for computing al the glottal area for the lower portion
      Delta_half_l = max([TBCMobj.xi_01-TBCMobj.xi_02, TBCMobj.xi_01])/2;
      xil_f = TBCMobj.xData(2)-Delta_half_l-TBCMobj.xl_col;
      al = 2*max([0, (1-TBCMobj.alpha_l)*TBCMobj.Lg*(xil_f+(1+TBCMobj.alpha_l)*Delta_half_l)]);
    end
...
%       xu_vproc = TBCMobj.xi_02;
      xu_vproc = max([0, TBCMobj.xi_02]); % Posterior position of the upper mass in the TBCM.
      x0u = xu_vproc/2; % Rest posiotion for the upper mass
      xu = TBCMobj.xData(1); %- 0.5*deltau0;
      
      if xu_vproc == 0
          alpha_u = 1*(xu<=0);
      else
          alpha_u = max([0, min([-(xu - abs(x0u) - xu_col_f)/abs(xu_vproc),1])]);
      end
...
% %               alpha_u = 0;
% %           else
%           alpha_u = max([0, min([-xu_f/deltau0,1])]);
% %           end
%       elseif deltau0 < 0
% %           if xu_f>0
%           alpha_u = max([0, min([(xu_f+deltau0)/deltau0,1])]);
% %           else
% %               alpha_u = 1;
% %           end
%       end
...
%       xl_vproc = TBCMobj.xi_01;
      xl_vproc = max([0, TBCMobj.xi_01]);% Posterior position of the lower mass in the TBCM.
      x0l = xl_vproc/2; % Rest posiotion for the lower mass
      xl = TBCMobj.xData(2); % - 0.5*deltal0;
      
      if xl_vproc == 0
          alpha_l = 1*(xl<=0);
      else
          alpha_l = max([0, min([-(xl - abs(x0l) - xl_col_f)/abs(xl_vproc),1])]);
      end
%       alpha_l = min([0.96 alpha_l]);
...
% %               alpha_l = 0;
% %           else
%           alpha_l = max([0, min([-xl_f/deltal0,1])]);
% %           end
%       elseif deltal0 < 0
% %           if xl_f>0
%           alpha_l = max([0, min([(xl_f+deltal0)/deltal0,1])]);
% %           else
% %               alpha_l = 1;
% %           end
%       end
